...
def find_cross_gxx(conf):
	...
	"""
	# call twice (kw "value") to avoid usage of preset CXX variable
	cxx = conf.find_program('%s-g++' % conf.env.CROSS_PLATFORM)
	cxx = conf.find_program('%s-g++' % conf.env.CROSS_PLATFORM, value=cxx,var='CXX')
